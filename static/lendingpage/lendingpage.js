...
app.controller("BooksReadersController", function ($scope, $http) {
    $scope.books = [];
    $scope.readers = [];
    ...
    $scope.initBooks = function () {
        $http.get(URLPREFIX + "/book/all").then(function (response) {
    ...
    $scope.initReaders = function () {
        $http.get(URLPREFIX + "/reader/all").then(function (response) {
    ...
    $scope.createABook = function (title, author) {
        $http.post(URLPREFIX + "/book/create", { "title": title, "author": author }).then(function (response) {
    ...
    $scope.createAReader = function (name) {
        $http.post(URLPREFIX + "/reader/create", { "name": name }).then(function (response) {
    ...
    $scope.lendBookToReader = function () {
        $http.post(URLPREFIX + "/action/lend", { "bookId": $scope.currentBookId, "readerId": $scope.currentReaderId }).then(function (response) {
    ...
    $scope.getBackSelectedBook = function () {
        ...
        $http.post(URLPREFIX + "/action/getBookBack/" + $scope.currentBookId).then(function (response) {
    ...
    $scope.updateCurrentReaderForCurrentBook = function () {
        ...
        $http.post(URLPREFIX + "/reader/currentBorrowerOfBook", { "bookId": $scope.currentBookId }).then(function (response) {
